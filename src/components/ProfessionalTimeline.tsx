...
export function ProfessionalTimeline({ assetName, duration, currentTime, playing, onPlayToggle, onSeek, onNotice }: Props) {
  ...
  // Memoize audio waveform heights to prevent allocating arrays and re-calculating Math.sin on every playhead time update render
  const waveformHeights = useMemo(() => {
    const count = Math.max(20, Math.floor(safeDuration * 3))
    return Array.from({ length: count }, (_, index) => `${22 + Math.abs(Math.sin(index * 1.7)) * 60}%`)
  }, [safeDuration])
  ...
  return <section className="pro-timeline">
    ...
    <div className="timeline-scroll">
      <div className="timeline-canvas" style={{ width: timelineWidth + 104 }}>
        ...
        <div className="pro-track audio-pro-track" style={{ width: timelineWidth }}><div className={`audio-pro-clip ${muted ? 'muted' : ''}`} style={{ width: safeDuration * pixelsPerSecond }}><Music2 size={11}/><span>{waveformHeights.map((height, index) => <i key={index} style={{ height }}/>)}</span></div><div className="timeline-playhead ghost" style={{ left: currentTime * pixelsPerSecond }}/></div>
      </div>
    </div>
  </section>
}
