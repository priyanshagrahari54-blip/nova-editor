...
export function ProfessionalTimeline({ assetName, duration, currentTime, playing, onPlayToggle, onSeek, onNotice }: Props) {
  ...
  // Memoize audio waveform heights so they aren't recalculated on every `currentTime` playhead tick during video playback.
  const audioBars = useMemo(() => Array.from({ length: Math.max(20, Math.floor(safeDuration * 3)) }, (_, index) => `${22 + Math.abs(Math.sin(index * 1.7)) * 60}%`), [safeDuration])
