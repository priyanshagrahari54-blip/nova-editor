...
export function ProfessionalTimeline({ assetName, duration, currentTime, playing, onPlayToggle, onSeek, onNotice }: Props) {
  ...
  // PERFORMANCE OPTIMIZATION: Memoize audio waveform bar elements so they aren't re-allocated
  // and re-reconciled on every single currentTime tick (30-60 fps) during timeline playback.
  const audioBars = useMemo(
    () => Array.from({ length: Math.max(20, Math.floor(safeDuration * 3)) }, (_, index) => (
      <i key={index} style={{ height: `${22 + Math.abs(Math.sin(index * 1.7)) * 60}%` }} />
    )),
    [safeDuration]
  )
