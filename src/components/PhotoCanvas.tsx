  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      if (!before) {
        ...
        if (settings.grain > 0) {
          // OPTIMIZATION: Batch grain rectangles into a single path to execute 1 draw call instead of up to 8,000 separate fillRect calls
          ...
          ctx.beginPath()
          for (let index = 0; index < count; index += 1) { seed = (seed * 233280 + 49297) % 233280; const x = seed / 233280 * canvas.width; seed = (seed * 233280 + 49297) % 233280; const y = seed / 233280 * canvas.height; ctx.rect(x, y, size, size) }
          ctx.fill()
          ctx.restore()
        }
