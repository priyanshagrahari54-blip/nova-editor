    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      if (!before) {
        ...
        if (settings.grain > 0) {
          ...
          // Batch up to 8,000 grain point rectangles into a single path call (`ctx.beginPath()` + `rect()`)
          // followed by a single `ctx.fill()`, instead of issuing thousands of separate `ctx.fillRect()` calls per frame.
          // Impact: Reduces canvas draw operations overhead significantly during live manipulation / continuous rendering.
