...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    ...
    // Batch up to 12,000 film grain particle rects into a single path call (`context.beginPath() ... context.fill()`)
    // to avoid excessive canvas draw call overhead per rendered frame during export.
