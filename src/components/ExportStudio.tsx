...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    // OPTIMIZATION: Batch up to 12,000 grain rectangles into a single path call with context.rect()
    // rather than dispatching thousands of individual fillRect() state/fill calls per export frame.
    // Impact: Accelerates master video export rendering speed when film grain is enabled.
