...
import { applyGrainOverlay, applySelectivePhotoAdjustments, buildPhotoFilter } from '../lib/photoAdjustments'
...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    applyGrainOverlay(context, width, height, settings.grain, frame % 2, frame % 2 ? '#fff' : '#111')
  }
