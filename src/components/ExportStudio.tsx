...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    // OPTIMIZATION: Batch grain rectangles into a single path to execute 1 draw call instead of up to 12,000 separate fillRect calls
    ...
    context.beginPath()
    for (let index = 0; index < count; index += 1) { seed = (seed * 233280 + 49297) % 233280; const x = seed / 233280 * width; seed = (seed * 233280 + 49297) % 233280; const y = seed / 233280 * height; context.rect(x, y, size, size) }
    context.fill()
    context.restore()
  }
