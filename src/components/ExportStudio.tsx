...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    ...
    // Batch up to 12,000 grain rectangles into a single path call (`context.beginPath()` + `rect()`)
    // followed by a single `context.fill()`, instead of thousands of individual `context.fillRect()` calls.
    // Impact: Accelerates master export and video frame rendering speeds when grain effect is enabled.
