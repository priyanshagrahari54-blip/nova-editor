...
import { applySelectivePhotoAdjustments, buildPhotoFilter, renderGrainOverlay } from '../lib/photoAdjustments'
...
function drawFinishing(context: CanvasRenderingContext2D, width: number, height: number, settings: EditorSettings, layers: EditorLayer[], frame = 0, applyPhotoTone = false) {
  ...
  if (settings.grain > 0) {
    renderGrainOverlay(context, width, height, settings.grain, frame % 2 !== 0, frame)
  }
